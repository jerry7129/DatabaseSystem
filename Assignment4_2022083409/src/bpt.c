...
typedef struct { // table state 구조체 정의
    int fd;
    H_P * hp;
    page * rt;
} T_S;
...
// 특정 table 작업을 위한 context switching
...
// 현재 table의 가장 왼쪽 leaf node의 offset 찾기
off_t get_leftmost_leaf() {
    if (rt == NULL || hp->rpo == 0) return 0;
    off_t cur = hp->rpo;
    page * p = load_page(cur);
    while (!p->is_leaf) {
        off_t next = p->next_offset; // interal node에선 next_offset이 가장 왼쪽 child를 가리킴
        free(p);
        cur = next;
        p = load_page(cur);
    }
...
void db_join() {
    context_switch(1);
    off_t t1_off = get_leftmost_leaf();
    if (t1_off == 0) return;
    page * p1 = load_page(t1_off);
    
    context_switch(2);
    off_t t2_off = get_leftmost_leaf();
    if (t2_off == 0) {
        free(p1);
        return;
    };
    page * p2 = load_page(t2_off);

    int idx1 = 0; int idx2 = 0;
    while (true) {
        if (idx1 >= p1->num_of_keys) { // table 1의 현재 page를 벗어난 경우 새로운 page를 load함.
            off_t next_off = p1->next_offset;
            free(p1);
            if (next_off == 0) { // table 1의 끝에 도달 -> join 종료
                free(p2);
                break;
            }
            context_switch(1); //fd를 table 1로 설정
            p1 = load_page(next_off);
            t1_off = next_off;
            idx1 = 0;
            continue;
        }
        if (idx2 >= p2->num_of_keys) { // table 2의 현재 page를 벗어난 경우 새로운 page를 load함.
            off_t next_off = p2->next_offset;
            free(p2);
            if (next_off == 0) { // table 1의 끝에 도달 -> join 종료
                free(p1);
                break;
            }
            context_switch(2); //fd를 table 2로 설정
            p2 = load_page(next_off);
            t2_off = next_off;
            idx2 = 0;
            continue;
        }

        int64_t k1 = p1->records[idx1].key;
        int64_t k2 = p2->records[idx2].key;
        if (k1 == k2) {
            printf("%ld,%s,%s\n", k1, p1->records[idx1].value, p2->records[idx2].value);
            idx1++; idx2++;
        } else if (k1 < k2)
            idx1++;
        else
