...
int db_insert(int64_t key, char * value) {
    ...
    while ( i < LEAF_MAX && leaf->records[i].key < key)
        i++;
    memcpy(tmp_records, leaf->records, i * sizeof(record)); // 정렬
    tmp_records[i] = new_rec;
    memcpy(&tmp_records[i + 1], &leaf->records[i], (LEAF_MAX - i) * sizeof(record));

    off_t new_lo = new_page();  // split을 위한 새로운 page 생성
    ...
    int split = cut(LEAF_MAX + 1); // split을 기준으로 양옆에 records 분배.
