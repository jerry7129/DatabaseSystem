...
4-1. .env 파일에서 지정한 DB_DATABASE 이름으로 데이터베이스를 생성해야 합니다.
    ```bash
    mysql -u root -p
    ```
    ```myql
    ...
    exit
    ```
4-2. SQL 덤프 파일(dump_db.sql)을 방금 생성한 libaray_db에 import 합니다.
    ```bash
    mysql -u root -p library_db < "[덤프_파일_경로]/dump_db.sql"
    ```
